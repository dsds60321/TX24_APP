...
(function (window, document) {
	...
	const FLAG_ATTR = 'data-layout-initialized';
	const rootElement = document.documentElement;
	if (rootElement.hasAttribute(FLAG_ATTR)) {
		return;
	}
	rootElement.setAttribute(FLAG_ATTR, 'true');

	const Layout = {
	init () {
		Layout.bindGlobalEvents();
		Layout.rebindDynamic(document);
		// 탭 기능
		Layout.TabManager.init();
		Layout.mobileSize(window.innerWidth);
	},

	bindGlobalEvents () {
		Layout.bindSidebarToggle();
	...
	bindSidebarToggle () {
		...
		sideMenuToggle.addEventListener('click', function() {
			Layout.sidebarHide(sideMenuToggle);
		});
	},
	...
	rebindDynamic (root) {
		...
		Layout.bindCollapseIconToggle(root);
		Layout.bindSelectToInput(root);
		Layout.bindEmailFocus(root);
		Layout.bindCardEdit(root);
	},

	bindCollapseIconToggle (root) {
	...
	bindCardEdit (root) {
		root.querySelectorAll('.flex-edit').forEach(function(button) {
			if (button.dataset.boundFlexEdit === 'true') {
				return;
			}
			button.dataset.boundFlexEdit = 'true';
			button.addEventListener('click', function() {
				Layout.cardEdit(button);
			});
		});
	},

	sidebarHide (sideMenuToggle) {
	...
	showTab (tabId, button) {
		const nav = button.closest('.nav-box');
		if (!nav) {
			return;
		}
	...
	checkAll (input) {
		...
		const inputs = table.querySelectorAll('td input.form-check-input');
		inputs.forEach(function(element) {
			element.checked = input.checked === true;
		});
	},
	...
	function bootstrapLayout() {
		if (bootstrapLayout.initialised) {
			return;
		}
		bootstrapLayout.initialised = true;
		Layout.init();
	}
