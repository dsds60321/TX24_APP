...
//         csrf 검증
        ...
        // 2차인증 준비
        RedisUtils.set(csrf, userData, Duration.ofMinutes(5).getSeconds());
    ...
    public void twoFactorAuth(String key) {
        ...
        // 5분 재설정
        RedisUtils.pexpire(key, Duration.ofMinutes(5).getSeconds() * 1000);
    }
     ...
     * 2차 인증 검증
     */
    public boolean confirmTwoFactorAuth(SharedMap<String, Object> param) {
        if (CommonUtils.hasEmptyValue(param, List.of("key", "code"))) {
            throw new TxException(TxResultCode.INVALID_REQUEST, "요청 파라미터가 유효하지 않습니다. 관리자에게 문의해주시기 바랍니다.");
        }
